...
class SourceAnswer(BaseModel):
    """A single source answer with URL and constant."""
    
    url: str = Field(description = "The URL of the source")
    answer: str = Field(description = "The detailed answer extarcted from that source")
...
async def run_perplexity_search(query: str) -> Tuple[str, Dict[int, str], Dict[int, str]]:
    ...
    return full_answer, answer_by_sources, citations
...
def append_perplexity_results(
    ...
    """
    Append results for a single query. 
    Returns next available global id.
    """
    with results_path.open("a", encoding="utf-8") as f:
        last_id = starting_id
        for local_id, url in citations.items():
            content = answer_by_source.get(local_id, "").lstrip()
            if not content:
                continue
...
async def run_queries(article_id: str, queries: List[str] ) -> None:
    if not queries:
        logger.warning("No queries supplied - nothing to do.")
        return
    ...
    logger.debug(f"\n Completed Perplexity research found. Results saved to {results_path}.")
    
def extract_perplexity_chunks(markdown: str) -> Dict[int, str]:
    """
    Return a msp {source_id: markdown_block} for each source block in the file.
...
def group_perplexity_by_query(chunks: Dict[int, str], selected_ids: List[int]) -> Dict[str, List[str]]:
    """
    Group source blocks by their query strings. Returns {query: [block, ...]}
    Only includes blocks whose source_id is in selected_ids.
    """
    query_to_blocks = {}
    for src _id in selected_ids:
